...
int iosf_mbi_available(void) {
    ...
    return ven == 0x8086U && dev == 0x0F00U;
}
...
int iosf_baytrail_scc_enable_pci_emmc(uint32_t* ctl_before_out, uint32_t* ctl_after_out) {
    ...
    if (!iosf_mbi_available()) return 0;
    if (!iosf_mbi_read(IOSF_PORT_SCC, IOSF_OP_READ_SCC, SCC_MMC_CTL, &ctl))
        return 0;
    ...
    if (after != ctl) {
        if (!iosf_mbi_write(IOSF_PORT_SCC, IOSF_OP_WRITE_SCC, SCC_MMC_CTL, after))
            return 0;
        /* Re-read to confirm. */
        if (!iosf_mbi_read(IOSF_PORT_SCC, IOSF_OP_READ_SCC, SCC_MMC_CTL, &after))
            return 0;
    }
