...
/*
 * Safe PCI enumeration for Braswell/UEFI: discover reachable buses by
 * following bridges from bus 0, then walk only those buses while honouring
 * the multifunction bit. Never probes phantom buses or phantom functions.
 *
 * Callback returns non-zero to stop early. Returns the number of devices
 * visited (functions with a valid vendor ID).
 */
typedef int (*pci_device_cb_t)(uint8_t bus, uint8_t slot, uint8_t func,
