...
#define DRIVER_LOG_CAPACITY 16384U
/* Drop oldest half when full so late USB2ROUTE / enum lines stay greppable. */
#define DRIVER_LOG_TRIM      (DRIVER_LOG_CAPACITY / 2U)
...
static void driver_log_trim_oldest(void) {
    uint32_t keep;
    uint32_t i;
    if (g_driver_log_len <= DRIVER_LOG_TRIM) return;
    keep = g_driver_log_len - DRIVER_LOG_TRIM;
    for (i = 0; i < keep; i++)
        g_driver_log[i] = g_driver_log[DRIVER_LOG_TRIM + i];
    g_driver_log_len = keep;
...
void driver_log(const char* msg) {
    ...
    for (uint32_t i = 0; msg[i]; i++) {
        if (g_driver_log_len + 1 >= DRIVER_LOG_CAPACITY)
            driver_log_trim_oldest();
        if (g_driver_log_len + 1 >= DRIVER_LOG_CAPACITY) {
            /* Pathological single huge write — keep newest byte. */
            g_driver_log_truncated = 1;
            g_driver_log[DRIVER_LOG_CAPACITY - 2] = msg[i];
            g_driver_log[DRIVER_LOG_CAPACITY - 1] = '\0';
            g_driver_log_len = DRIVER_LOG_CAPACITY - 1;
            g_driver_log_version++;
            continue;
        }
...
void driver_log_dump(void (*write)(const char*)) {
    ...
    if (g_driver_log_truncated)
        write("\n[driver-log] log truncated (oldest half dropped; newest kept).\n");
}
