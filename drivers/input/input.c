...
void input_set_usb_pointer_active(int active) {
    ...
    if (usb_pointer_active) {
        if (active_pointer != INPUT_DEVICE_USB_TOUCHPAD)
...
void input_set_usb_pointer_kind(int is_touchpad) {
    uint32_t flags = irq_save_disable();
    if (usb_pointer_active) {
        active_pointer = is_touchpad ? INPUT_DEVICE_USB_TOUCHPAD
...
static int is_device_allowed(input_device_t device) {
    /* USB mouse and I2C touchpad both may drive the cursor. PS/2 is suppressed
     * whenever either higher-quality pointer source is live. */
    if (device == INPUT_DEVICE_PS2_MOUSE &&
        (usb_pointer_active || i2c_touchpad_active)) {
        return 0;
    }
    return 1;
}
...
void input_remove_device(input_device_t device) {
    ...
    /* If the removed device was the active pointer, revert + release buttons
     * so a held click does not stick after unplug. */
    if (device == INPUT_DEVICE_USB_MOUSE || device == INPUT_DEVICE_USB_TOUCHPAD) {
        usb_pointer_active = 0;
    }
