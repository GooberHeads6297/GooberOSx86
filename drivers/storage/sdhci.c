...
static int sdhci_wait_reset_clear(uint32_t base, uint8_t mask) {
    for (uint32_t spins = 0; spins < 200000U; spins++) {
...
static int sdhci_wait_interrupt(uint32_t base, uint32_t mask, uint32_t* out_status) {
    for (uint32_t spins = 0; spins < 400000U; spins++) {
        ...
        if (status & mask) {
            if (out_status) *out_status = status;
            mmio_write32(base, SDHCI_REG_INT_STATUS, mask);
            return 1;
        }
    }
...
static int sdhci_write_data_block(uint32_t base, uint16_t cmd_index, uint32_t argument, const void* buffer, uint32_t* status_out) {
    ...
    if (!sdhci_wait_interrupt(base, SDHCI_INT_BUF_WRITE_READY, &status)) {
        if (status_out) *status_out = status;
        sdhci_recover_error(base, SDHCI_RESET_DATA);
        return 0;
    }
...
static int sdhci_init_card(uint8_t bus,
    ...
    if (caps0 == 0xFFFFFFFFU || host_version == 0xFFFFU) return 0;
    ...
    // For a successfully initialized MMC/eMMC device, the SD socket-style
    // write-protect signal is not a reliable install gate.
...
int sdhci_probe_pci_controller(uint8_t bus,
                               ...
                               uint32_t bar0,
                               sdhci_probe_result_t* out) {
    return sdhci_init_card(bus, slot, func, bar0, out);
}
...
static int sdhci_rw_sector_common(sdhci_state_t* state, uint32_t lba, void* buffer, int write) {
    ...
    if (write) return sdhci_write_data_block(base, MMC_CMD_WRITE_BLOCK, argument, buffer, &status);
