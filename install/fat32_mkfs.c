...
static int inst_blast_partition_template(const storage_device_info_t* dev,
    ...
    for (lba = 0; lba < to_write; lba++) {
        uint32_t pct;
        if (install_payload_read_fat_template_sector(lba, sector) != 0)
            return -1;
        if (storage_write_sector(dev, INSTALL_PARTITION_START_LBA + lba, sector) != 0)
            return -1;
        /* Report every 5% so the shell stays alive (~20 lines over ~15s). */
        pct = ((lba + 1U) * 100U) / to_write;
        if (pct != last_pct && (pct % 5U) == 0U) {
            last_pct = pct;
            install_report_pct("copying FAT32 template", lba + 1U, to_write);
        }
    }
    return 0;
}
