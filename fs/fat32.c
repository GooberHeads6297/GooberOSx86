...
int fat32_mount_device_loose(int device_index, int part_index) {
    return fat32_mount_device_internal(device_index, part_index, 0);
}

void fat32_unmount(void) {
    if (!g_vol.mounted) return;
    /* Avoid calling fat32_sync here if dirty — sync may need a live device.
     * Best-effort flush; ignore failure on vanishing media. */
    if (g_vol.fat_dirty) (void)fat32_sync();
    memset(&g_vol, 0, sizeof(g_vol));
    g_mount_desc[0] = '\0';
    g_desktop_dir = NULL;
    current_dir = NULL;
