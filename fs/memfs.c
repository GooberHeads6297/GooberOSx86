...
static FileEntry root_files_static[] = {
    { "file1.txt", (uint8_t*)file1_data, sizeof(file1_data) - 1, 0, 0, 0, 0 },
    { "log.txt", (uint8_t*)log_data, sizeof(log_data) - 1, 0, 0, 0, 0 }
};

static FileEntry docs_files_static[] = {
    { "readme.txt", (uint8_t*)readme_data, sizeof(readme_data) - 1, 0, 0, 0, 0 }
};

static FileEntry etc_files_static[] = {
    { "config.ini", (uint8_t*)config_data, sizeof(config_data) - 1, 0, 0, 0, 0 }
};

static Directory base_root_dir[] = {
    { "/", root_files_static, sizeof(root_files_static) / sizeof(FileEntry),
      sizeof(root_files_static) / sizeof(FileEntry),
      NULL, NULL, 0, 0, 0, 0 },
    { "docs", docs_files_static, sizeof(docs_files_static) / sizeof(FileEntry),
      sizeof(docs_files_static) / sizeof(FileEntry),
      &base_root_dir[0], NULL, 0, 0, 0, 0 },
    { "etc", etc_files_static, sizeof(etc_files_static) / sizeof(FileEntry),
      sizeof(etc_files_static) / sizeof(FileEntry),
      &base_root_dir[0], NULL, 0, 0, 0, 0 }
};
...
void memfs_init(void) {
    for (size_t i = 0; i < MAX_OPEN_FILES; i++) {
        ...
        handles[i].offset = 0;
        handles[i].fat_cluster = 0;
        handles[i].fat_cluster_base = 0;
    }
        ...
        if (dir_name_equal(filename, dir->files[i].name)) {
            for (size_t h = 0; h < MAX_OPEN_FILES; h++) {
                if (!handles[h].used) {
                    ...
                    handles[h].offset = 0;
                    handles[h].fat_cluster = 0;
                    handles[h].fat_cluster_base = 0;
                    return &handles[h];
                }
            }
            return 0;
...
int memfs_dir_create(Directory* dir, const char* filename) {
    ...
    for (size_t i = 0; i < dir->file_count; i++) new_files[i] = dir->files[i];
    ...
    new_files[new_count - 1].fat32 = 0;
    new_files[new_count - 1].fat_cluster = 0;
    new_files[new_count - 1].is_dir = 0;
    if (!is_static_files(dir->files)) kfree(dir->files);
    dir->files = new_files;
...
int memfs_create(const char* filename) {
    return memfs_dir_create(current_dir, filename);
}
...
int memfs_dir_write(Directory* dir, const char* filename,
    ...
    if (!target) {
        if (memfs_dir_create(dir, filename) != 0) return -1;
...
int memfs_write(const char* filename, const uint8_t* data, size_t size) {
    return memfs_dir_write(current_dir, filename, data, size);
}
...
int memfs_delete(const char* filename) {
    ...
    if (new_count == 0) {
        if (!is_static_files(current_dir->files)) kfree(current_dir->files);
        current_dir->files = 0;
    ...
    for (size_t i = 0, j = 0; i < current_dir->file_count; i++) {
        if (i != idx) new_files[j++] = current_dir->files[i];
    }
    if (!is_static_files(current_dir->files)) kfree(current_dir->files);
...
int memfs_dir_create_dir(Directory* dir, const char* dirname) {
    ...
    for (size_t i = 0; i < dir->child_count; i++) new_children[i] = dir->children[i];
    ...
    new_children[dir->child_count].fat32 = 0;
    new_children[dir->child_count].fat_cluster = 0;
...
int memfs_create_dir(const char* dirname) {
    return memfs_dir_create_dir(current_dir, dirname);
}
...
int memfs_dir_rename(Directory* dir, const char* old_name, const char* new_name) {
    if (!dir || !old_name || !new_name || old_name[0] == '\0' || new_name[0] == '\0')
        return -1;
    if (strlen(new_name) >= MAX_NAME_LEN) return -1;
    if (dir_name_equal(old_name, new_name)) return 0;
...
int memfs_rename(const char* old_name, const char* new_name) {
    return memfs_dir_rename(current_dir, old_name, new_name);
}
    ...
    for (size_t i = 0; i < current_dir->child_count; i++) {
        if (dir_name_equal(dirname, current_dir->children[i].name)) {
            for (size_t f = 0; f < current_dir->children[i].file_count; f++) {
                if (current_dir->children[i].files[f].owned &&
                    current_dir->children[i].files[f].data) {
                    kfree(current_dir->children[i].files[f].data);
                }
            }
            if (current_dir->children[i].files &&
                !is_static_files(current_dir->children[i].files)) {
...
Directory* memfs_get_desktop_dir(void) {
    ...
    if (!found) {
        if (memfs_dir_create_dir(root, "Desktop") != 0) return NULL;
